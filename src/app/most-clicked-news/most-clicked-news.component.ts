...
// src/app/most-read-news/most-read-news.clicked.ts
...
@Component({
  selector: 'app-most-clicked-news',
  standalone: true,
  imports: [
    CommonModule,
    RouterModule,
  ...
  templateUrl: './most-clicked-news.component.html',
  styleUrls: ['./most-clicked-news.component.css']
})
export class MostClickedNewsComponent implements OnInit {
  mostClickedNews: Haber[] = [];
...
//burası çalışma zamanında en çok okunan haberleri çekmek için kullanılır
  ...
  ngOnInit(): void {
    this.getMostClickedNews();
  }

  getMostClickedNews(): void {
    this.newsService.getMostClickedNews().subscribe({
      next: (data) => {
        this.mostClickedNews = data;
        this.isLoading = false;
        this.error = null;
      },
      error: (err) => {
        console.error('En çok tıklanan haberler çekilirken hata oluştu:', err);
